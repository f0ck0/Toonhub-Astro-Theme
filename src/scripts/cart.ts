...
/** 促销是否 active:false 时折扣恒为 0、不显示活动文案 */
let promoEnabled = true;
/** 当前折扣率(如 50 = 50% off),来自后端配置 */
let promoValue: number | null = null;

async function fetchRemote(): Promise<{ items: CartItem[]; discountTotal: number } | null> {
  ...
  try {
    ...
    const data = (await res.json()) as {
      items?: CartItem[];
      cart?: { discount_total?: number };
    };
    return {
      items: Array.isArray(data.items) ? data.items : [],
      discountTotal: Number(data.cart?.discount_total) || 0,
    };
  } catch {
    return null;
  }
...
async function applyAutoPromo(cartId: string, code: string) {
...
async function refresh(): Promise<void> {
  ...
  if (info.code && remote && remote.items.length >= 2 && remote.discountTotal <= 0) {
    ...
    if (cartId) {
      ...
      if (again) {
        items = again.items.length || !local.length ? again.items : local;
...
function renderTotals(): void {
  ...
  // 促销停用时不显示任何折扣与活动文案
  const discount = promoEnabled ? (serverDiscount > 0 ? serverDiscount : bogoDiscount(items, promoValue ?? 50)) : 0;
  ...
  // 折扣行标签文案动态化(折扣率随后台配置)
  const promoText = promoEnabled && promoValue != null ? `Buy 1 get 2nd ${promoValue}% off (auto-applied)` : "";
  for (const el of $$<HTMLElement>("[data-promo-label]")) el.textContent = promoText;
  ...
  // Nudge copy: one unit → invite a second; two or more → show the saving.(促销停用则不显示)
  const nudgeKey = !promoEnabled ? "" : units === 1 ? "single" : discount > 0 ? "saving" : "";
  const pct = promoValue != null ? promoValue : 50;
  const messages = {
    single: `Add one more figure — the 2nd is ${pct}% off.`,
    saving: `You're saving ${formatMoney(discount, currency)} with Buy 1 get 2nd ${pct}% off.`,
  } as const;
...
export function snapshotOrder(email: string) {
  const discount = bogoDiscount(items, promoValue ?? 50);
  const order = {
